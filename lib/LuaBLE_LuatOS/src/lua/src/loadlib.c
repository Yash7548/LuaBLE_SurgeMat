...
static int checkload (lua_State *L, int stat, const char *filename) {
  if (stat) {  /* module loaded successfully? */
    ...
    return luaL_error(L, "error loading module '%s' from file '%s':\n\t%s",
        ...
        #define LUAT_MODULE_SEARCH_PATH   "/spiffs", "/spiffs/%s.lua","/%s.luac", "/%s.lua", "/luadb/%s.luac", "/luadb/%s.lua", "/lua/%s.luac", "/lua/%s.lua"
           NULL
        #endif
      #elif defined(LUAT_LITTLEFS)
      #ifndef LUAT_MODULE_SEARCH_PATH
      #define LUAT_MODULE_SEARCH_PATH   "/littlefs/%s.lua", "/littlefs/%s.luac","/%s.luac", "/%s.lua", "/luadb/%s.luac", "/luadb/%s.lua", "/lua/%s.luac", "/lua/%s.lua"
